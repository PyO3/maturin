...
pub trait WheelRepairer {
    ...
    ///
    /// Returns the indices into `audited` of artifacts whose bytes were
    /// modified in place. The caller uses this to decide, **per artifact**,
    /// whether the staged file can still be renamed back to the cargo
    /// output path after the wheel is written.
    ///
    ...
    fn patch_editable(&self, _audited: &[AuditedArtifact]) -> Result<Vec<usize>> {
        Ok(Vec::new())
    }
