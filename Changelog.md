...
* Perf: defer the post-staging artifact copy and only restore the cargo output path via an O(1) `rename` when no auditwheel patching was needed, avoiding multi-second post-compile copies on ext4 and other non-CoW filesystems ([#3111](https://github.com/PyO3/maturin/issues/3111))
