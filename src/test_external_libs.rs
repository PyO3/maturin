...
mod test_external_libs_repeated_build {
    ...
    fn test_artifact_copy_creation() {
        ...
        fs::create_dir_all(temp_artifact_path.parent().unwrap()).expect("Failed to create temp dir");
        ...
        let original_content_after = fs::read(&artifact_path).expect("Failed to read original after modification");
        assert_eq!(original_content, original_content_after);
        
        // Verify copy is modified
        let copy_content_after = fs::read(&temp_artifact_path).expect("Failed to read copy after modification");
